 ...
 */
export function createPaymentUrl({ config, data }: { config: GlobalConfig; data: Record<string, unknown> }): URL {
...
export function calculateSecureHash({
  ...
  return crypto.createHmac(hashAlgorithm, secureSecret).update(Buffer.from(data, bufferEncode)).digest('hex')
}
...
export function verifySecureHash({
  ...
  const calculatedHash = crypto.createHmac(hashAlgorithm, secureSecret).update(Buffer.from(data, 'utf-8')).digest('hex')
