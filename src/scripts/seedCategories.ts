...
const categoriesData = [
  {
    ...
    subcategories: [
      {
        name: 'Nike',
        subcategories: ['Air Max', 'Air Force'],
      },
      {
        name: 'Adidas',
        subcategories: ['SL 72', 'Samba', 'Spezial', 'Gazelle', 'Superstar', 'Adizero'],
      },
    ],
  },
  {
    ...
    subcategories: [
      {
        name: 'Nike',
        subcategories: ['Sneakers', 'Jersey', 'Shorts', 'Jackets', 'Hoodies'],
      },
      {
        name: 'Adidas',
        subcategories: ['Sneakers', 'Jersey', 'Shorts', 'Jackets', 'Hoodies'],
      ...
      {
        name: 'Arsenal',
        subcategories: ['Jersey', 'Sneakers', 'Jersey', 'Shorts', 'Jackets', 'Hoodies'],
      },
      {
        name: 'Juventus',
        subcategories: ['Jersey', 'Sneakers', 'Jersey', 'Shorts', 'Jackets', 'Hoodies'],
  ...
  },
  {
    ...
    subcategories: [
      {
        name: 'Nike',
        subcategories: ['T-Shirts', 'Jersey', 'Tracksuits', 'Backpacks'],
      },
      {
        name: 'Adidas',
        subcategories: ['T-Shirts', 'Jersey', 'Tracksuits', 'Smiley World'],
      },
      {
        name: 'Arsenal',
        subcategories: ['Jersey', 'Tracksuits'],
      },
      {
        name: 'Juventus',
        subcategories: ['Jersey', 'Tracksuits'],
    ...
    for (const cat of categoriesData) {
      ...
      for (const sub of cat.subcategories) {
        ...
        for (const productType of sub.subcategories) {
          const productTypeCat = await Category.findOne({
            name: productType,
...
export const fixCategoryStructure = async () => {
  for (const cat of categoriesData) {
    ...
    for (const sub of cat.subcategories) {
      ...
      const wrongTeams = await Category.find({
        name: sub.name,
        parentCategory: { $ne: parent._id },
      });
      for (const wrong of wrongTeams) {
        await Category.deleteOne({ _id: wrong._id });
        console.log(
          `Deleted wrong team category: ${sub.name} (parentCategory: ${wrong.parentCategory})`,
        );
      }
      if (!team) {
        team = await new Category({
          name: sub.name,
          parentCategory: parent._id,
          status: 'active',
        }).save();
      ...
      for (const productType of sub.subcategories) {
        ...
        const wrongProds = await Category.find({
          name: productType,
          parentCategory: { $ne: team._id },
        });
        for (const wrong of wrongProds) {
          await Category.deleteOne({ _id: wrong._id });
          console.log(
            `    Deleted wrong product type: ${productType} (parentCategory: ${wrong.parentCategory})`,
          );
        }
        if (!prod) {
          await new Category({
