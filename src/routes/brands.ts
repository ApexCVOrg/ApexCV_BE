...
router.get('/', async (_req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: 'Lỗi khi lấy danh sách thương hiệu: ' + (error as Error).message })
  }
})
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: 'Lỗi khi lấy thông tin thương hiệu: ' + (error as Error).message })
  }
})
