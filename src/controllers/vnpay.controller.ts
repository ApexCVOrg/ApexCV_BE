  ...
  try {
    ...
    if (!orderData && !userId) {
      ...
      return res.status(400).json({
        status: 'fail',
        message:
          'Không tìm thấy thông tin đơn hàng và người dùng. Vui lòng thực hiện lại quá trình thanh toán.',
    ...
    console.log(
      '[VNPAY Return] Order items details:',
      JSON.stringify(orderData.orderItems, null, 2),
    );
    ...
    const populatedOrderItems = await Promise.all(
      orderData.orderItems.map(async (item: any, index: number) => {
        ...
        console.log(
          `[VNPAY Return] Found product for item ${index + 1}:`,
          product
            ? {
                id: product._id,
                name: product.name,
        ...
        const sizeWithSku = item.size.map((sizeItem: any) => ({
          ...
          sku:
            sizeItem.sku ||
            `${product?.name || 'product'}_${sizeItem.size}_${sizeItem.color || 'default'}`
              .replace(/\s+/g, '_')
              .toUpperCase(),
        }));
    ...
    console.log(
      '[VNPAY Return] Final populated items:',
      JSON.stringify(populatedOrderItems, null, 2),
    );
    ...
    console.log(
      '[VNPAY Return] Order items summary:',
      savedOrder.orderItems.map((item: any, index: number) => ({
        index: index + 1,
        productName: item.productName || item.name,
        quantity: item.quantity,
        size: item.size[0]?.size || 'N/A',
        ...
        const filteredCartItems = userCart.cartItems.filter((cartItem: any) => {
          ...
          if (paidQuantity !== undefined) {
            ...
            if (cartItem.quantity <= paidQuantity) {
              console.log(
                `[VNPAY Return] Removing cart item: ${cartKey} (quantity: ${cartItem.quantity})`,
              );
              return false; // Xóa item này
            } else {
              // Nếu quantity trong cart > quantity đã thanh toán, giảm quantity
              console.log(
                `[VNPAY Return] Reducing quantity for cart item: ${cartKey} from ${cartItem.quantity} to ${cartItem.quantity - paidQuantity}`,
              );
    ...
    console.log(
      '[VNPAY Return] Sending success response:',
...
async function createOrderFromVnpayData(req: Request, res: Response) {
  try {
    ...
    console.log(
      '[VNPAY Return] Sending success response from VNPAY data:',
...
export const handleIpn = (req: Request, res: Response) => {
  try {
    ...
    if (!isPaymentSuccess) {
      ...
      if (vnpTxnRef) {
        try {
          PendingOrder.deleteMany({ 'orderData.sessionId': vnpTxnRef })
            .then(() => {
