...
export const createRefundRequest = async (req: Request, res: Response) => {
  try {
    ...
    if (req.files && Array.isArray(req.files)) {
      images = (req.files as Express.Multer.File[]).map(file => `${process.env.API_BASE_URL || 'http://localhost:5000'}/uploads/${file.filename}`);
    }
    ...
    const refund = await RefundRequest.create({
      orderId, userId, txnRef, amount, reason, status: 'pending', images
    });
...
export const acceptRefundRequest = async (req: Request, res: Response) => {
  try {
    ...
    const refundData = {
      ...
      vnp_TransactionDate: parseInt(refund.createdAt.toISOString().replace(/[-T:.Z]/g, '').slice(0, 14)),
      vnp_CreateBy: String(managerId),
      vnp_CreateDate: parseInt(now.toISOString().replace(/[-T:.Z]/g, '').slice(0, 14)),
    ...
    try {
      ...
      let refundReason = refund.reason;
      let shopName = 'NIDAS';
      let supportPhone = '0123456789'; // Có thể lấy từ settings nếu có
      if (order) {
